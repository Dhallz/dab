...
- **Notifier state classes** (`*State` in `views/*/…_state.dart`): Extensions on that state type **must** live in the **same file**, **below** the state class (e.g., `extension OnAdminState on AdminState` at the bottom of `admin_state.dart`). Do **not** use a sibling `extensions/on_*_state.dart` file for state projections.
