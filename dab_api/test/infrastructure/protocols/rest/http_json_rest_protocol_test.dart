  ...
  group('getJsonList', () {
    test('returns list on 200 JSON array', () async {
      ...
      when(
        ...
        (_) async => http.Response('[{"sha":"abc"}]', 200),
      );

      final list = await protocol.getJsonList(uri, headers: {'Accept': 'application/json'});
      expect(list, hasLength(1));
      expect(list.first, isA<Map>());
      verify(() => mockClient.get(uri, headers: {'Accept': 'application/json'})).called(1);
    });
