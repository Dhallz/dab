...
/// [ARCH: DOMAIN]
/// ROLE: OAuth provider catalog lookups on a provider id or GitLab instance URL.
extension OnString on String {
  /// True when [this] is a per-user OAuth provider (not Phorge, Slack, or Discord).
  bool get isOauthUserProvider =>
      kOauthProviderIds.contains(trim().toLowerCase());

  /// GitLab web origin (no `/api/v4`) used for `/oauth/authorize` and `/oauth/token`.
  String gitlabWebBase() {
    var raw = trim();
  ...
  /// Resolves OAuth endpoints for [this] provider id. [instanceUrl] is used for GitLab.
  OauthProviderSpec? oauthSpecFor({String instanceUrl = ''}) {
    switch (trim().toLowerCase()) {
      ...
      case 'gitlab':
        final base = instanceUrl.gitlabWebBase();
