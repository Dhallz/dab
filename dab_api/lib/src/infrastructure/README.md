...
| Folder | Contents |
|---|---|
| `sources/` | Provider I/O (`*_source` implementing `AbsIActivityPort`, catalogs, Discord Gateway) |
...
1. **Sources**: Specialized fetchers (e.g., `PhorgeTaskSource`) that implement `AbsIActivityPort` and handle raw I/O and protocol management for a specific data type.
...
Sources are data-type specific (e.g., Task, Revision). If you are adding a new platform, implement `AbsIActivityPort` in a new Source and link it to its corresponding Domain Mapper in the `ConnectorRegistry`.
