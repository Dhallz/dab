...
import 'noop_push_wake_client.dart';

/// [ARCH: INFRASTRUCTURE]
/// ROLE: FCM HTTP v1 data-only inbox wakes. Never sets a `notification` block.
class FcmHttpV1PushWakeClient implements AbsIPushWakeClient {
  FcmHttpV1PushWakeClient({
  ...
  Future<void> sendWake({
    ...
    if (!data.isInboxWakeData) return;
  ...
  static AbsIPushWakeClient resolve(
    Config config, {
    http.Client? client,
  }) {
    if (config.isDevelopment) return const NoopPushWakeClient();
    return tryParse(config.fcmServiceAccountJson, client: client) ??
        const NoopPushWakeClient();
  }

  /// Builds a client from a Google service-account JSON string.
  ///
  /// Returns null when [raw] is not a usable service account (caller uses noop).
  static FcmHttpV1PushWakeClient? tryParse(
    String raw, {
    http.Client? client,
  }) {
    ...
    return FcmHttpV1PushWakeClient(
      projectId: projectId,
