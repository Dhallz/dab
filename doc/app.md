...
| **Settings** | User personalization | Theme, language, Island Bar, Explorer cache, and **Connected accounts**. GitHub, GitLab, Linear, Jira, and Bitbucket use **Connect with {provider}** (browser OAuth). After Jira is connected, Settings shows a **project picker** that writes instance `projectKeys` (Connect also seeds keys from whoami so polling works immediately). After Linear is connected, Settings shows a **team picker** that writes instance `teamKeys` (shared watch list for issue fetch). Phorge still uses a Conduit token. Slack/Discord bots are configured in Admin. Tokens are never stored on the device. |
