...
Shared glass panels use `DabGlassSurface` (`AppGlassTheme`). Chrome icons use `AppIcons`. On desktop, Explorer pairs the feed with a foldable left sidebar split into **Directory**, **Activities**, and **Providers** checklist sections. **Insights** adds filterable analytics cards/charts. **Admin** metrics are projected from `AdminState` via `OnAdminState.islandBarModel`.
...
| **Control kit** | [core/widgets](file:///Users/dhallz/git/dab/dab_app/lib/presentation/core/widgets/) | Shared chrome: `DabGlassSurface`, `DabToggleChip`, `DabIslandStat`, `ActivityProviderIcon`, `IslandBar`. |
