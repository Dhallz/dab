...
| **Explorer** | Historical activity browser | Chronological strip with selectable timeframe; on load selects **all** directory users (`selectedUserIds`) then narrows to the signed-in user when matched (same breadth as Insights for the logged-in path). Sidebar **Providers** and **Activities** lists only include admin-**activated** providers with a **green** (successful) connection test; filters reconcile when activation or connection health changes via `appNotifierProvider` (home shell keeps the branch alive). **Clear cache and refresh** (calendar header) evicts ObjectBox rows for the active day or range across all browsable providers, then refetches from provider APIs. |
| **Insights** | Filterable behavior analytics | KPI + trend + provider/type/user breakdowns with details table; search uses **`authoredOnly=false`** for team-aggregate provider queries (Explorer keeps **`true`**). Provider and activity-type filters match Explorer: only admin-**activated** providers with a **green** connection test appear and stay synced with `appNotifierProvider`. |
| **Settings** | User personalization | Dynamic forms for theme variant (Light, DAB, grayscale Dark), runtime language selection, per-view Island Bar item visibility (Dashboard/Explorer/Insights), and **Explorer cache** controls (date range + provider checklist to clear local ObjectBox activity/coverage rows) |
| **Admin Console** | System administration | Multi-tab dashboard: **Provider Config** cards group fields into **Core / Live / Polling** sections, each with its own status light. The main card light aggregates **Live + Polling** only (green = both ok, orange = one ok, red = neither). **Try** calls `POST /admin/configs/test` and stores the full section report; when no recent live ingest exists, the API runs an internal **webhook test delivery** (validates Live signing secrets) so Live can turn green without a real provider event. Live section includes an editable **Webhook Endpoint URL** (`settings.webhookUrl`); when unset, the field defaults to `{public_api_url}/integrations/{provider}/webhook` (Slack: `/integrations/slack/events`). Discord notes Gateway-based ingest (no webhook field). Explorer/Insights still require aggregate **green** (`ViewStatus.success`). Also: **Identity Management** (create/update links; **Update Identity Link** dialog includes **Delete Link** with confirmation → `DELETE /admin/identities/:id`), **Security** (domain validation, org timezone, `public_api_url`), and **Add User** via `AdminNotifier.createUser`. |
