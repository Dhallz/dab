...
- **Reports:** org-calendar date (today in v1), Following toggle, Copy / Download Markdown.
...
| **Home** | Shell around home tabs | `HomeView` + `StatefulNavigationShell` (`/home/dashboard`, `/home/reports`, `/home/explorer`, `/home/insight`, `/home/admin`). Not a data screen. |
...
| **Reports** | Personal daily-report authoring | Org-calendar **today** × (Directed-at-me **union** authored-by-me). `ReportsNotifier` hydrates `GET /activities/live` and subscribes to `/ws` itself (does not read `dashboardNotifierProvider`). Authored rows come from `GET /activities/search?authoredOnly=true` for the signed-in user. Lines merge by **subject key** (provider + object + occurrence — commit sha / comment id / message ts, not git Follow `owner/repo\|branch`). Default all included; optional DAB-only note per included line. Persist via `GET/PUT /users/me/day-reports/:date` (debounced). Generate Markdown (time, provider, headline, note) for clipboard + `.md` download — no LLM. Following live rows stay off unless the toolbar toggle is on. Not a Dashboard clone (no archive/Follow required). |
