...
Shared glass panels use `DabGlassSurface` (`AppGlassTheme`). Chrome icons use `AppIcons`. Multi-select sidebar filters (Explorer, Insights, Admin, Settings cache) use shared `SelectionTile`. Directory chrome is shared (`DirectoryPanel` in `lib/presentation/core/widgets/`): Explorer and Insights include Users/Groups plus group CRUD; Reports is users-only and single-select. Custom groups live in one shared `directoryGroupsProvider` so a create/rename/delete on Explorer is the same list on Insights. Demo seed writes Engineering / Product / Design / Leadership into that API list. On desktop, Explorer and Insights share `FilterSidebar` — collapsible **Directory**, **Activities**, and **Providers** sections (Directory fills leftover height). Reports uses the same `FilterSidebar` with **Directory** (managers/admins) plus a **Reports** date list for the selected person. **Insights** adds filterable analytics cards/charts. **Admin** metrics are projected from `AdminState` via `OnAdminState.islandBarModel`.
...
| **Auth** | Auth gate (`AuthView`) | Form via `AuthFormNotifier`; session in `authNotifierProvider`. Includes an **API URL** field (default `http://localhost:9080` or last saved origin) applied before login/register. The register toggle is shown **only while `isSystemConfigured` is false** (bootstrap): the first registered user becomes admin; afterwards self-registration is closed and accounts are created from the Admin Console. |
...
- `ServiceLocator` — custom singleton in `lib/services/service_locator.dart` (not GetIt). This is the only place dependencies are wired. REST and WebSocket origins default from `ApiBaseUrl` (`--dart-define=DAB_API_BASE`, local Docker). The auth form **API URL** field persists the last origin (`ApiOriginStorage`) and retargets Dio / the live socket before login. Hosted API: [deployment.md](./deployment.md).
...
- **Identity linking:** Mentions and watches target the signed-in user through `user_identities` (Connect whoami or Admin link). The client `User.linkedProviderIds` list is a Directory hint only. Screenshot mock mode (`DAB_ENABLE_MOCK` / `APP_ENV=development`) and seed teammates fill that hint with active providers so user tiles do not show “Hasn't connected {provider}”.
...
**Launcher icons** follow [flutter_launcher_icons](https://pub.dev/packages/flutter_launcher_icons): configuration lives in `dab_app/flutter_launcher_icons.yaml` (the CLI default; see package guide). Source image: `dab_app/assets/branding/app_icon.png` — keep it **opaque RGB** and full-bleed (no alpha). macOS treats icons with transparency as a glyph on a glass plate, which shows up as a light margin in the Dock. `remove_alpha_ios` composites onto `background_color_ios` (`#1F1F4B`). To scaffold a new config file, run `dart run flutter_launcher_icons:generate` (use `-o` to overwrite). Platform folders (`ios/`, `android/`, `macos/`, etc.) must exist before the tool can write icons—run `flutter create . --platforms=...` in `dab_app/` when setting up a fresh clone where those directories are absent.

**macOS window title:** keep `PRODUCT_NAME = dab_app` in `macos/Runner/Configs/AppInfo.xcconfig` (spaces in that value break `Runner.xcodeproj`). The traffic-light bar and Dock use `CFBundleDisplayName` / `CFBundleName` in `macos/Runner/Info.plist` and `MainFlutterWindow.title` — all **Dev Activity Board**. `MaterialApp.router` `title` is `appWindowTitle` (same string). After `flutter create --platforms=macos`, restore those display-name keys — that folder is gitignored.

**macOS ObjectBox sandbox:** sandboxed macOS builds must declare the `dab.objectbox` application group in `macos/Runner/DebugProfile.entitlements` and `Release.entitlements` (`com.apple.security.application-groups`), and pass the same id as `macosApplicationGroup` when opening the store (`ObjectBoxStore.macosApplicationGroupId`). Without it, ObjectBox fails with `StorageException` / `Operation not permitted` (POSIX semaphores). Also keep `com.apple.security.network.client` so the API client can reach the backend. Restore both after `flutter create --platforms=macos`.
