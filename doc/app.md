...
- **Dashboard:** last-sync and the show-archived chip. Live/archived counts sit in the desktop sidebar with provider health (two-line `DabIslandStat`); on mobile they are inline in the toolbar.
...
- Sidebar content is live/archived counts plus provider health chips from `DashboardState`.
...
- The toolbar hosts last-sync and the show-archived `DabToggleChip`. Live/archived counts are in the desktop sidebar (and the mobile toolbar). A reconnect notice can appear in the feed after a WS gap.
