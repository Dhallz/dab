...
Home branches (**Dashboard**, **Explorer**, **Insights**, **Admin**) use a compact **view toolbar** (`ViewToolbar` in `lib/presentation/core/widgets/view_toolbar.dart`) — one left-packed row (horizontal scroll if needed), no glass capsule. Explorer pins refresh on the trailing edge. Controls share the same density (`DabToggleChip`, compact `DabIslandStat`):

- **Dashboard:** Timeline / Category / Provider chips, last-sync, and the show-archived chip. Live/archived counts sit in the desktop sidebar with provider health (two-line `DabIslandStat`); on mobile they are inline in the toolbar.
...
| **Dashboard** | Personal inbound inbox | Initial hydration from `GET /activities/live` (user scope; never `scope=global`), then live updates via authenticated `/ws` (`ACTIVITY_RECEIVED` to the signed-in recipient only). Renders a **Live Now** feed of activities **directed at you** (or on objects you Follow) with per-item Archive/Unarchive triage and Follow/Unfollow on Phorge, Jira, Linear, Slack, and Discord cards. Git commit cards have no Follow control. Archive flags live on `activities:user:{id}`. The toolbar switches **Timeline / Category / Provider** layout (`DashboardFeedMode`, session-only), last-sync, and the show-archived chip; live/archived counts are in the desktop sidebar (and the mobile toolbar). |
...
| **Settings** | User personalization | Theme, language, Explorer cache, and **Connected accounts**. Route stays `/settings` (not a home tab) with `DabMeshBackground` and home-like back chrome to Dashboard. GitHub, GitLab, Linear, Jira, and Bitbucket use **Connect with {provider}** (browser OAuth). After connect, **link the identity** so mentions and watches can target you. After Jira is connected, Settings shows a **project picker** that writes instance `projectKeys` (Explorer ingest allow-list; Connect also seeds keys from whoami). After Linear is connected, Settings shows a **team picker** that writes instance `teamKeys`. After GitHub/GitLab/Bitbucket is connected, Settings shows a **personal repo picker** (optional branches) stored on the user credential — that list targets the Dashboard git inbox. Phorge still uses a Conduit token. Slack/Discord bots are configured in Admin. Tokens are never stored on the device. |
...
- **Desktop layout** uses the shared glass `AppSidebar` plus a main column with the dashboard toolbar and `DashboardLiveFeed` (see `DashboardViewDesktop`).
- Sidebar content is live/archived counts plus provider health chips from `DashboardState`. Health is one row per **Admin-activated** provider, colored from `appNotifierProvider.providerConnectionStatuses` (green until a connection test/credential fails). Quiet providers with no live events stay green. `lastEventAt` is optional metadata and does not drive color.
- Main feed defaults to a single **Live Now** timeline of `visibleActivities`, honouring the `showArchivedActivities` toggle. Timeline mode draws a vertical **HH:mm rail** to the left of each `DashboardActivityCard` (org timezone via `orgLocalFromUtc`), with a date crumb when the org-calendar day changes. **Category** mode groups that same subset into one glass container per `ActivityCategory` (Commit, Revision, Task, Message, Generic), including quiet types with an empty hint. **Provider** mode uses one container per Admin-activated provider from `providerHealth` (quiet providers keep an empty column) plus any extra names that appear only on the feed. Category and Provider use a **responsive masonry** grid (`MasonryGridView`): column count is `min(itemCount, 3, width ~/ 320)` so leftover viewport width **widens** tiles instead of adding skinny kanban columns, and each container sizes to its content.
- Each `DashboardActivityCard` exposes an Archive action (trashcan) or Unarchive (refresh) routed through **`dashboardNotifierProvider`** with **optimistic UI** and rollback on failure. Followable cards also expose Follow / Following (bookmark) next to Archive; git commit cards omit it. Follow pins hydrate with the live feed from `GET /users/me/follows`. Dashboard does not reuse Explorer’s grouped `ActivityCard`. The same cards are used in every feed mode.
...
- The toolbar hosts exclusive Timeline / Category / Provider `DabToggleChip`s, last-sync, and the show-archived chip. Live/archived counts are in the desktop sidebar (and the mobile toolbar). A reconnect notice can appear in the feed after a WS gap.
