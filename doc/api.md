...
- **Live ingest:** Provider ingest use cases share `IngestionResult` and `LiveIngestPersister` (SQL insert + Redis fan-out + ingest-success timestamp). Application talks to Redis/Presence/JWT through `AbsILiveFeedStore`, `AbsIPresenceBroadcaster`, and `AbsIAccessTokenIssuer`. After WebSocket emit, `ActivityLivePublisher` sends a data-only inbox wake (`type`, `lane`, `activityId` — never title/body) when the recipient has no WS session and has registered device tokens.
- **Activity query use cases:** `GetRecentActivities`, `SearchActivities`, and `FetchRemoteActivities`.
- **`GetLiveActivities`:** Serves **`GET /activities/live`** from Redis lists only (no Postgres). `limit` is 1–250 (default 50). Dashboard and Reports request 150 so a dense screenshot seed can fill Directed and Following. Explorer historical browsing uses **`SearchActivities`** (**`GET /activities/search`**), not Redis.
...
|---|---|---|
| `ActivityController` | `/activities*`, `/ws`, `/integrations/*/webhook`, `/integrations/slack/events`, `/mock/*` | Historical feed (`/activities`), **dashboard `GET /activities/live`** is **Redis-backed only** (optional `?includeArchived=true`; omit `scope` or use `user` for the signed-in inbound inbox; `scope=global` is still accepted by the API but unused by Dashboard). Live-feed triage (`POST /activities/live/:id/archive`, `POST /activities/live/:id/unarchive`), provider push receivers (`/integrations/slack/events`, `/integrations/{github,phorge,jira,linear,gitlab,bitbucket,figma}/webhook`) — HMAC/shared-secret via `AbsIWebhookRequestAuthenticator`, then ingest use case. Historical **`GET /activities/search`** (UnifiedActivityFetcher polling — no Postgres merge — plus `AbsIDemoActivityStore` union for screenshot seeds). WebSocket `/ws`. Unauthenticated `POST /mock/activity` injects a generic Mock row. JWT `POST /mock/demo-day` seeds a screenshot day (Postgres + Redis live + Follow pins + daily report + `demo:search:{userId}:{date}`) when `DAB_ENABLE_MOCK=true` or `APP_ENV=development`. `team: true` creates/reuses 20 named teammates with varied titles and cross-user directed rows for Explorer/Insights. Archived live entries stay in Redis (nightly purge). `ActivityPurgeScheduler`. `ActivityLivePollScheduler` is retained for DI but does **not** publish authored poll rows into the live inbox. |
...
| `MetadataController` | `/metadata/*`, `/admin/configs*`, `/admin/system-settings` | Public bootstrap status/configs (includes `deploymentMode` and daily-report lock `dailyReportLockOffsetDays` / `dailyReportLockTime`), provider metadata list, provider capability matrix (`/metadata/capabilities`), admin provider config save/test (`POST /admin/configs/test` returns Core/Live/Polling section report; Live green = Redis `live:last_ingest:{providerId}` within 7 days), system settings (domain validation toggle + allowed domain + `public_api_url` for OAuth callbacks and webhooks + `deployment_mode` `managed`/`individual` + global daily-report deadline `daily_report_lock_offset_days` / `daily_report_lock_time`) |
| `UserController` | `/users/*` | User directory plus self-serve credentials (`GET/PUT/DELETE /users/me/credentials`, `POST /users/me/credentials/:provider/test`, `POST /users/me/credentials/:provider/oauth/start`, `GET/PUT /users/me/credentials/{jira,linear,github,gitlab,bitbucket}/projects` for Jira/Linear instance allow-lists and personal git inbox watches, `GET /users/me/credentials/{github,gitlab,bitbucket}/branches` for the Settings branch picker), Dashboard object Follow pins (`GET/PUT/DELETE /users/me/follows` with `{ providerId, objectKey, title?, url? }` for Phorge, Jira, Linear, Slack, Discord, Figma `file_key`, and git `owner/repo|branch`; `GET /users/me/follows/candidates?q=` for the Following and Reports pickers — typed `q` substring-matches display titles (`[T12] …`)), personal daily reports (`GET /users/me/day-reports` and `GET /users/:id/day-reports` list saved dates newest first; `GET/PUT /users/me/day-reports/:date` — empty draft is 200 with empty `lines`, not 404; PUT is **403** after the Admin report deadline; notes stay in DAB; `GET /users/:id/day-reports/:date` is read-only for **manager**/**admin**, 403 for **standard** unless `id` is self), and mobile device tokens (`PUT/DELETE /users/me/device-tokens` with `{ platform, token }` / `{ token }`). Secrets are encrypted at rest (`DAB_CREDENTIALS_KEY`); list/test/oauth-start never echo tokens. OAuth tokens are stored as the existing provider secret keys plus `tokenType=oauth`. Jira Cloud access tokens expire in about an hour; listing projects refreshes them via `offline_access` before calling Jira. Slack/Discord bots are instance `ProviderConfig` fields (Admin), not per-user Settings paste. |
...
- **`GET /activities/search` query `startDate` / `endDate`:** Bare `YYYY-MM-DD` (no TZ) is interpreted as **organization calendar days** in the configured `system_timezone` (default `UTC`), converted to UTC instants for provider polling and DB queries (`ActivityController` + `OrgCalendar`). **`dab_app`** sends the same `YYYY-MM-DD` strings derived from Explorer/Insights picker dates in the org timezone (`ActivitySearchQueryMapper.toRemoteQueryParameters`), and client-side filtering uses the same org-day semantics. **`authoredOnly`:** **`dab_app`** Explorer keeps **`true`** for personal-scope browsing; **Insights** uses **`false`** for team analytics so connectors can apply broader retrieval (e.g. Phorge sprint/global paths). **`providers`:** optional comma-separated provider config ids (`slack`, `phorge`, …). Omitted or empty means every **active** registered connector. Explorer fans out one search per selected provider so a slow connector does not block other rows. Concurrent searches for the same connector/window share one upstream poll (`UnifiedActivityFetcher` in-flight coalesce) so Figma/Slack/GitHub are not hit N times. Connector polls still require a linked identity; `POST /mock/demo-day` rows are unioned from Redis `demo:search:{userId}:{date}` **after** that gate so a screenshot day can fill Explorer without provider HTTP.
